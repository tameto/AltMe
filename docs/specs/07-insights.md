...
- **MBTI表示**: `profiles.mbti_type` が設定済みの場合、Big Fiveグラフの下にMBTIタイプを表示
  - 表示形式: 「MBTI: INTJ」のようにタイプ名 + 簡易説明
  - 未設定時: 「MBTIを設定する」リンク（設定画面へ遷移）
  - 設定画面でのみ入力・変更可能（Twin Infoタブでは表示のみ）
...
- [ ] MBTI設定済みの場合、MBTIタイプが表示されること
- [ ] MBTI未設定時に「MBTIを設定する」リンクが表示されること
